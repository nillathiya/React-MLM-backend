...
async function addToWallet(uCode, fromWallet, toWallet, amount) {
    try {
        ...
        await Promise.all([
            transaction.save(),
            common.mangeWalletAmounts(uCode, toWallet, amount),
            common.mangeWalletAmounts(uCode, fromWallet, -amount)
        ]);
    } catch(e) {
    ...
    try {
        ...
        for (let order of ordersData) {
            ...
            if (payable > 0) {
                ...
                walletUpdates.push(
                    common.mangeWalletAmounts(user._id, 'roi', payable),
    ...
    try {
        ...
        for (let counter = 1; counter <= Math.min(level, plan.value.length); counter++) {
            ...
            if (payable > 0) {
                ...
                walletUpdates.push(
                    common.mangeWalletAmounts(currentUCode, source, payable),
                    walletSettingsData.wallet && common.mangeWalletAmounts(currentUCode, walletSettingsData.wallet, payable * 0.25),
                    common.mangeWalletAmounts(currentUCode, 'weekly_pool', payable * 0.45),
                    common.mangeWalletAmounts(currentUCode, 'monthly_pool', payable * 0.30)
                );
            }
        }
    ...
    try {
        ...
        for (let counter = 1; counter <= Math.min(level, plan.value.length); counter++) {
            ...
            if (payable > 0) {
                ...
                walletUpdates.push(
                    common.mangeWalletAmounts(currentUCode, 'roi_level_commission', payable),
                    walletSettingsData.wallet && common.mangeWalletAmounts(currentUCode, walletSettingsData.wallet, payable * 0.25),
                    common.mangeWalletAmounts(currentUCode, 'weekly_pool', payable * 0.45),
                    common.mangeWalletAmounts(currentUCode, 'monthly_pool', payable * 0.30)
                );
